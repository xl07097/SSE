const Functional = require('./functionalMenuModel');
const ObjectID = require('mongodb-core').BSON.ObjectID;

let operation = {
    insert: function (params, callback) {
        let user = new Functional({
            title: params.title,
            pid: params.pid,
        ...
        operation.select({ "title": params.title }, function (err, data) {
            if (err) {
                callback(err);
            } else {
                if (data.length) {
                    ...
                    Functional.save(function (err, res) {
                        if (err) {
                            callback(err);
                        } else {
                            res.status = true;
    ...
    update: function (params, data, callback) {
         ...
         */
        Functional.update(params, data, function (err, res) {
            if (err) {
                callback(err);
            } else {
    ...
    updateById: function (id, data, callback) {
         ...
         */
        Functional.findByIdAndUpdate(id, data, function (err, result) {
            if (err) {
                callback(err);
            } else {
                callback(null, result)
    ...
    delete: function (params, callback) {
        ...
        Functional.findByIdAndRemove(params.id, function (err, res) {
            if (err) {
                callback(err);
            } else {
                callback(null, result);
    ...
    select: function (param, callback) {
         ...
         *
         */
        Functional.find(param, function (err, res) {
            if (err) {
                callback(err);
            } else {
                callback(null, res);
    ...
    selectById: function (params, callback) {
         ...
         */
        Functional.findById(params.id, function (err, result) {
            if (err) {
                callback(err)
            } else {
                callback(null, result)
    ...
    selectByReg: function (params, callback) {
         ...
         * 模糊查询
         */
        Functional.find(params, function (err, res) {
            if (err) {
                callback(err)
            } else {
                callback(null, result)
    ...
    selectCount: function (params, callback) {
        ...
        Functional.countDocuments(wherestr, function (err, result) {
            if (err) {
                callback(err)
            } else {
                callback(null, result)
    ...
    selectPage: function (params, callback) {
        ...
        operation.selectCount(params, function (err, count) {
            if (err) {
                ...
                Functional.find(condition).skip(skipnum).limit(size).sort(sort).exec(function (err, result) {
                    if (err) {
                        callback(err)
                    } else {
                        callback(null, {
